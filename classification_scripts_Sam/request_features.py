...
'''
def add_idf_to_word_dataframe(request_matrix):
...
def add_tf_idf_to_request_dataframe(request_matrix, idf_i_list):
    for j in range(request_matrix.shape[0]):
        for i in range(len(idf_i_list)):
            ...
            request_matrix[j, 3*i] = tf_idf_i_j
...
if __name__=="__main__":
    ...
    print("step 3")
    word_list_df, idf_i_list = add_idf_to_word_dataframe(request_matrix=matrix)
    print("step 4")
    matrix = add_tf_idf_to_request_dataframe(matrix, idf_i_list)
    print("step 5")
    request_df = fuse_request_matrix_into_df(request_df, matrix, columns)
    print("step 6")
    request_df.to_csv("request_df_expanded.csv")
    word_list_df.to_csv("word_list_df_expanded.csv")